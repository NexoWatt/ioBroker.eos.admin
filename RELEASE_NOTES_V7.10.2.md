...
Die Release-Lifecycle-Skripte kompilieren das Backend vor `npm pack` und `npm publish`. Zusätzlich verfolgt der Backend-Runtime-Test die lokale Modulabhängigkeitskette ab `build/main.js`, führt einen Syntaxcheck aus und verifiziert die Aufnahme jeder Runtime-Datei in das npm-Artefakt. Damit wird insbesondere ein fehlendes `build/lib/eosRequestSecurity.js` vor einer Veröffentlichung erkannt.
