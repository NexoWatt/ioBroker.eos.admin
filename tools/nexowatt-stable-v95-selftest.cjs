...
must(Array.isArray(pkg.files) && pkg.files.includes('tools/nexowatt-repair-eos-admin-update.cjs'),'npm package must include the installed update-repair helper');
must(!pkg.files.includes('tools/'),'npm runtime package must not ship the complete source-only validation toolset');
for (const stale of ['README_STABILITY_V7.9.94.md','RELEASE_NOTES_V7.9.94.md','PUBLISH_STABLE_V7.9.94.md','INSTALL_TEST_V7.9.94.md','RELEASE_ACCEPTANCE_V7.9.94.md']) must(!pkg.files.includes(stale),`stale missing package file entry remains: ${stale}`);
for (const current of ['NEXOWATT_EOS_STABLE_7.9.94.md','README_STABILITY_V7.9.95.md','RELEASE_NOTES_V7.9.95.md','PUBLISH_STABLE_V7.9.95.md','INSTALL_TEST_V7.9.95.md','RELEASE_ACCEPTANCE_V7.9.95.md']) must(pkg.files.includes(current),`required package file is not listed: ${current}`);
for (const sourceOnly of ['MERGE_UPDATE_README_V7.9.95.md','MERGE_UPDATE.cmd','MERGE_UPDATE.ps1']) must(!pkg.files.includes(sourceOnly),`source-only merge helper must not be included in the installed npm runtime: ${sourceOnly}`);
