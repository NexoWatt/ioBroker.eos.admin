...
if (!/^7\.9\.\d+(?:-rc\.\d+)?$/.test(version)) fail(`unexpected version ${version}`);
...
const scripts = ['eos-policy-client.js','eos-branding-sanitizer.js','nexowatt-native-shell.js','eos-native-security.js','eos-basic-settings.js','eos-role-ui.js','eos-assistant.js'];
...
for (const rel of ['js/eos-role-bootstrap.js','js/eos-policy-client.js','js/eos-branding-sanitizer.js','js/nexowatt-native-shell.js','js/eos-native-security.js','js/eos-basic-settings.js','js/eos-role-ui.js','js/eos-assistant.js','js/eos-manual-write-policy.js','css/nexowatt-native-shell.css']) {
...
for (const marker of ['getEosFirstLoginPasswordState','/nexowatt/account/first-password','passwordSetupVersion','logoutRequired']) if (!read('build/lib/web.js').includes(marker)) fail(`first-login web marker missing: ${marker}`);
for (const marker of ['ensureEosRoleModel','system.group.installateur','system.group.endkunde','ensureEosSystemObjectAccess','ensureEosSmartHomeEnumAccess']) if (!read('build/main.js').includes(marker)) fail(`role backend marker missing: ${marker}`);
