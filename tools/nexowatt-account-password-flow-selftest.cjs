...
for(const code of [source,built]){
 ...
 must(code.includes("command === 'changePassword'")&&code.includes('password administration is Service-only'),'native password command role guard missing');
}
...
must(role.includes("route !== 'tab-users'")&&role.includes('filterNativeUsersPage'),'native Users page is not blocked for Installer/End User');
console.log('[NexoWatt EOS standard password flow] OK (initial password on creation, Admin-only native password editor, no forced reset flow)');
