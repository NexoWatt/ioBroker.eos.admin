...
function must(ok,msg){if(!ok){console.error(`[NexoWatt EOS stable v94] ${msg}`);process.exit(1);}}
const pkg=readJson('package.json');
const io=readJson('io-package.json');
must(pkg.version==='7.9.94','package version must be 7.9.94');
must(io.common.version==='7.9.94','io-package version must be 7.9.94');
...
const overlay=fs.readFileSync(path.join(root,'adminWww/static/js/nexowatt-stable-v94.js'),'utf8');
...
must(html.includes('nexowatt-stable-v94.js'),'stable runtime overlay is not loaded');
const bad=[];
...
console.log('[NexoWatt EOS stable v94] OK');
