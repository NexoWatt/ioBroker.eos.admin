...
for(const code of [source,built]){
 for(const marker of ['system.group.installateur','system.group.endkunde','ensureEosRoleModel',"command === 'changePassword'",'password target outside EOS role scope',"users: { list: true, read: true, write: false"] )must(code.includes(marker),`backend marker missing: ${marker}`);
 must(!code.includes("personal password setup is required"),'socket still blocks standard password mode');
}
must(role.includes("route === 'tab-users'")&&role.includes('filterNativeUsersPage'),'native role-scoped user page missing');
must(role.includes("v7101-native-overview-standard-users"),'role UI version missing');
...
console.log('[NexoWatt EOS role access] OK (admin all, installer scoped accounts, customer self password)');
