...
async function ensureEosStableInitialAccounts(adapter) {
    for (const account of EOS_STABLE_ACCOUNTS) {
        ...
        let created = false;
        if (!object) {
            await adapter.setForeignObjectAsync(id, { type: 'user', common: { name: account.user, enabled: true, password: '' }, native: {} }, { user: 'system.user.admin' });
        ...
        if (!object) continue;
        const previous = object.native || {};
        const previousAccount = previous.nexowattEosAccount || {};
        await adapter.extendForeignObjectAsync(id, { native: {
            ...previous, nexowattManaged: true, nexowattRole: account.role,
            ...
            nexowattResetRequested: false, eosResetRequested: false,
            nexowattEosAccount: { ...previousAccount, passwordInitialized: true, passwordSetupVersion: 1,
                passwordInitializationVersion: 1, forcePasswordChange: false,
                passwordlessFirstLoginAllowed: false, standardPasswordMode: true },
...
class Admin extends adapter_core_1.Adapter {
    ...
    onMessage = (obj) => {
        ...
        else if (obj.command === 'eos:completeFirstLogin') {
            const message = obj.message || {};
            const user = String(message.user || '').replace(/^system\\.user\\./, '').trim().toLowerCase();
            const password = String(message.password || '');
            if (!['installer', 'guest', 'user'].includes(user) || !password) {
                if (obj.callback) {
                    this.sendTo(obj.from, obj.command, { error: 'Ungültige Anfrage' }, obj.callback);
                }
                return;
            }
            void this.checkPasswordAsync(user, password)
                .then(async (result) => {
                const valid = Array.isArray(result) ? result[0] === true : !!result;
                if (!valid) {
                    throw new Error('Das neue Passwort konnte nicht bestätigt werden');
                }
                const id = `system.user.${user}`;
                const account = await this.getForeignObjectAsync(id);
                if (!account) {
                    throw new Error('Benutzerkonto nicht gefunden');
                }
                const eosNative = account.native || {};
                await this.extendForeignObjectAsync(id, {
                    native: {
                        ...
                        nexowattPasswordChangedAt: new Date().toISOString(),
                    },
                }, { user: 'system.user.admin' });
                if (obj.callback) {
