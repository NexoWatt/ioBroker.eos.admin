...
const stableVersion = (value) => /^\d+\.\d+\.\d+$/.test(String(value || '').trim());
function isNexoWattRepositoryEntry(name, entry) {
    const adapterName = normalizedName(entry?.name || name).toLowerCase();
    if (adapterName === 'eos-admin' || adapterName.startsWith('nexowatt-') || adapterName.startsWith('eos-'))
        return true;
...
class NexoWattStableUpdateManager {
    ...
    scheduleStartupReconcile() {
        if (this.startupTimer || this.stopped)
            return;
        this.startupTimer = setTimeout(() => {
            this.startupTimer = undefined;
            if (!this.stopped) {
                void this.reconcile('startup').catch(error => this.adapter.log.warn(`[NexoWatt stable updates] Startup reconciliation failed: ${error instanceof Error ? error.message : error}`));
            }
    ...
    async readPersisted() {
        ...
        try {
            ...
            if (typeof enabledState?.val === 'boolean')
    ...
    statusFromPersisted(enabled, state) {
        return {
            enabled,
    ...
    async readRepositorySelection(systemConfig) {
        ...
        const candidates = Object.entries(rows).map(([name, repo]) => {
            const entries = repo?.json && typeof repo.json === 'object' ? repo.json : {};
            const stableNames = Object.entries(entries)
                .filter(([entryName, entry]) => stableVersion(entry?.version) && isNexoWattRepositoryEntry(entryName, entry))
                .map(([entryName, entry]) => normalizedName(entry?.name || entryName))
                .filter(Boolean);
        ...
        if (!candidates.length)
            throw new Error('nexowattStableRepositoryNotFound');
        candidates.sort((a, b) => {
    ...
    async enable(state, reason) {
        ...
        if (!managedNames.length)
            throw new Error('noInstalledNexoWattStableAdapters');
        if (this.stopped)
        ...
        for (const name of managedNames) {
            ...
            if (!(name in state.previousPolicies)) {
                ...
                state.previousPolicies[name] = current == null ? null : current;
            }
        ...
        if (activeRepo[0] !== selected.name) {
            if (!state.previousActiveRepo)
                state.previousActiveRepo = [...activeRepo];
            state.ownsActiveRepoOrder = true;
            common.activeRepo = [selected.name, ...activeRepo.filter((name) => name !== selected.name)];
            systemConfigChanged = true;
        }
    ...
    async disable(state, reason) {
        for (const [name, previous] of Object.entries(state.previousPolicies || {})) {
            ...
            if (current?.common?.automaticUpgrade === POLICY)
                await this.writeAdapterPolicy(name, previous);
        }
        const systemConfig = await this.adapter.getForeignObjectAsync('system.config');
        if (systemConfig?.common) {
            let systemConfigChanged = false;
    ...
    scheduleRepositoryRefresh(repository, state) {
        if (!this.adapter.sendToHostAsync || !this.adapter.host || this.stopped)
            return;
        if (this.refreshTimer)
            clearTimeout(this.refreshTimer);
        this.refreshTimer = setTimeout(() => {
            ...
            void this.adapter.sendToHostAsync?.(this.adapter.host, 'getRepository', { repo: repository, update: true })
                ...
                .catch(error => {
                if (!this.stopped)
                    this.adapter.log.warn(`[NexoWatt stable updates] Repository refresh failed: ${error instanceof Error ? error.message : error}`);
            });
