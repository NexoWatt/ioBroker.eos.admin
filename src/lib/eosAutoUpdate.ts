...
interface NexoWattAutoUpdateAdapter {
    ...
    extendForeignObjectAsync: (id: string, object: any, options?: any) => Promise<any>;
...
export class NexoWattStableUpdateManager {
    ...
    async start(): Promise<NexoWattStableUpdateStatus> {
        const status = await this.reconcile('startup');
        if (!this.timer) {
            this.timer = setInterval(() => void this.reconcile('interval'), RECONCILE_INTERVAL_MS);
            this.timer.unref?.();
        }
        return status;
    }
    ...
    async getStatus(refresh = false): Promise<NexoWattStableUpdateStatus> {
        ...
        return this.statusFromNative(instance?.native || {});
    }

    async setEnabled(enabled: boolean): Promise<NexoWattStableUpdateStatus> {
        await this.adapter.extendForeignObjectAsync(this.instanceId, { native: { eosNexoWattAutoUpdate: enabled } }, { user: ADMIN_USER });
        return this.reconcile(enabled ? 'enabled' : 'disabled');
    }
    ...
    private statusFromNative(native: Record<string, any>): NexoWattStableUpdateStatus {
        const state = (native.eosNexoWattAutoUpdateState || {}) as PersistedState;
        return {
            enabled: native.eosNexoWattAutoUpdate !== false,
    ...
    private async reconcileInternal(reason: string): Promise<NexoWattStableUpdateStatus> {
        ...
        const enabled = native.eosNexoWattAutoUpdate !== false;
        const state = clone((native.eosNexoWattAutoUpdateState || {}) as PersistedState);
        try {
            ...
            return this.statusFromNative({ eosNexoWattAutoUpdate: enabled, eosNexoWattAutoUpdateState: state });
        }
    }
    ...
    private async enable(state: PersistedState, reason: string): Promise<NexoWattStableUpdateStatus> {
        ...
        if (!auto.defaultPolicy) auto.defaultPolicy = 'none';
        ...
        this.scheduleRepositoryRefresh(selected.name, state);
        this.adapter.log.info(`[NexoWatt stable updates] Enabled for ${managedNames.join(', ')} from repository "${selected.name}" (${reason})`);
        return this.statusFromNative({ eosNexoWattAutoUpdate: true, eosNexoWattAutoUpdateState: state });
    }
    ...
    private async disable(state: PersistedState, reason: string): Promise<NexoWattStableUpdateStatus> {
        ...
        if (systemConfig?.common) {
            ...
            if (auto && state.selectedRepository) {
                ...
                if (auto.repositories[state.selectedRepository] === true) {
                    if (state.previousRepositoryEnabled === null || state.previousRepositoryEnabled === undefined) delete auto.repositories[state.selectedRepository];
                    else auto.repositories[state.selectedRepository] = state.previousRepositoryEnabled;
                }
        ...
        this.adapter.log.info(`[NexoWatt stable updates] Disabled and previous policies restored (${reason})`);
        return this.statusFromNative({ eosNexoWattAutoUpdate: false, eosNexoWattAutoUpdateState: state });
    }
    ...
    private async persist(enabled: boolean, state: PersistedState): Promise<void> {
        await this.adapter.extendForeignObjectAsync(this.instanceId, {
            native: { eosNexoWattAutoUpdate: enabled, eosNexoWattAutoUpdateState: state },
        }, { user: ADMIN_USER });
    }

    private scheduleRetry(): void {
        if (this.retryTimer) clearTimeout(this.retryTimer);
        this.retryTimer = setTimeout(() => void this.reconcile('retry'), 60_000);
        this.retryTimer.unref?.();
    }

    private scheduleRepositoryRefresh(repository: string, state: PersistedState): void {
        if (!this.adapter.sendToHostAsync || !this.adapter.host) return;
        if (this.refreshTimer) clearTimeout(this.refreshTimer);
        this.refreshTimer = setTimeout(() => {
            void this.adapter.sendToHostAsync?.(this.adapter.host!, 'getRepository', { repo: repository, update: true })
                .then(async () => {
                    const current = await this.adapter.getForeignObjectAsync(this.instanceId);
                    if (current?.native?.eosNexoWattAutoUpdate === false) return;
                    state.lastRepositoryRefresh = Date.now();
                    await this.persist(true, state);
                })
                .catch(error => this.adapter.log.warn(`[NexoWatt stable updates] Repository refresh failed: ${error instanceof Error ? error.message : error}`));
